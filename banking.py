...
class User:
    ...
    def show_details(self):
        return f"Thank you, {self.age} year's old, {self.name.title()}"
...
def options(user_two):
    print("-----Thank you for creating a bank account-----")
    print("Here are the list options, please choose the numbers you want")
    while True:
        option_choice = int(input("1) check the Balance \n2) Withdraw\n3) Deposit\n4) See total withdraws\n5)See Total Deposits\n6)Quit \n"))
        if option_choice == 1:
            ...
            if option_choice == 2 and user_two != None:
                wd = input(f"{user_two.name} would you like to withraw? Yes or No: ")
                if wd.lower() == "yes":
            ...
            print("Thank your for using PYVault Bank.")
            return False
            break
        else:
            print("Please a choose a number from 1 to 6.")

def bank_creation(name):
    balance = float(input(f"{name.title()},how much do you like to deposit? ₹ "))
    return balance

while True: 
    print("{BOLD}-----welcome to PYVault Bank-----")
    ...
    if new_user.lower() == 'yes':
        name = input("Enter the second person name: ")
